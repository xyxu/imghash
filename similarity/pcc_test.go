...
var pccUInt8Tests = []struct {
	...
	{hashtype.UInt8{94, 111, 255, 58, 0, 98, 37, 3, 111, 71, 72, 50, 129, 131, 75, 86}, hashtype.UInt8{121, 106, 129, 93, 134, 157, 144, 131, 89}, 0, similarity.ErrNotSameLength},
}

func TestPCCUInt8(t *testing.T) {
	for _, tt := range pccUInt8Tests {
		t.Run(fmt.Sprintf("%v", tt.res), func(t *testing.T) {
			res, err := similarity.PCC(tt.hash1, tt.hash2)
			if !res.Equal(tt.res) {
...
func ExamplePCC() {
	...
	res1, err := similarity.PCC(hash1, hash2)
	if err != nil {
		panic(err)
	}
	res2, err := similarity.PCC(hash1, hash3)
	if err != nil {
		panic(err)
	}
	fmt.Println(res1)
...
var pccFloat64Tests = []struct {
	...
	{hashtype.Float64{0.007840559221738475, 2.2034498359157744e-07, 8.684939637320095e-09, 1.3195340752596305e-08, 1.361994698531565e-16}, hashtype.Float64{0.002662394563829838, 8.300904725182467e-08, 1.4588163678577022e-09, 1.7799284933778596e-09, 1.972364977368551e-18}, 1.0000000052112052, nil},
	{hashtype.Float64{0.003941680044858082, 4.051369372556929e-07, 2.811861997021288e-11, 5.705112955860052e-10}, hashtype.Float64{0.0025293177387043795, 1.4356598602797603e-08, 1.8704987719836307e-10, 1.6439303696445385e-12, -4.875692849536562e-24, -6.192153867552066e-17, 2.841197567643476e-23}, 0, similarity.ErrNotSameLength},
}
