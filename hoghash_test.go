...
var hogHashDistanceTests = []struct {
	...
	{"assets/lena.jpg", "assets/cat.jpg", 0.509984487750516, 32, 32, imghash.Bilinear, 8, 9},
	{"assets/lena.jpg", "assets/monarch.jpg", 0.3223155603833341, 32, 32, imghash.Bilinear, 8, 9},
	{"assets/baboon.jpg", "assets/cat.jpg", 0.4056824811720108, 32, 32, imghash.Bilinear, 8, 9},
	{"assets/peppers.jpg", "assets/baboon.jpg", 0.28314243977711895, 32, 32, imghash.Bilinear, 8, 9},
	{"assets/tulips.jpg", "assets/monarch.jpg", 0.17623016930102986, 32, 32, imghash.Bilinear, 8, 9},
}
...
func TestNewHOGHash_invalidSize(t *testing.T) {
	_, err := imghash.NewHOGHash(imghash.WithSize(0, 0))
...
func TestNewHOGHash_invalidCellSize(t *testing.T) {
	_, err := imghash.NewHOGHash(imghash.WithCellSize(0))
	if !errors.Is(err, imghash.ErrInvalidCellSize) {
		t.Errorf("expected imghash.ErrInvalidCellSize, got %v", err)
	}
}

func TestNewHOGHash_invalidNumBins(t *testing.T) {
	_, err := imghash.NewHOGHash(imghash.WithNumBins(0))
	if !errors.Is(err, imghash.ErrInvalidNumBins) {
		t.Errorf("expected imghash.ErrInvalidNumBins, got %v", err)
	}
}

func TestNewHOGHash_customCellSize(t *testing.T) {
	hog, err := imghash.NewHOGHash(imghash.WithSize(32, 32), imghash.WithCellSize(16))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	img, err := imghash.OpenImage("assets/cat.jpg")
...
func TestNewHOGHash_customNumBins(t *testing.T) {
	hog, err := imghash.NewHOGHash(imghash.WithSize(32, 32), imghash.WithNumBins(18))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	img, err := imghash.OpenImage("assets/cat.jpg")
